...
function loadPlaces(map) {
  var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 44.95;
  var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 34.10;
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    // zoom the map to fit our bounds
...
function typeAhead(search) {
  ...
  searchInput.on('keyup', function (e) {
    ...
    if (current) {
      switch (e.keyCode) {
        case 40:
          next = current.nextElementSibling || items[0];break;
        case 38:
          next = current.previousElementSibling || items[items.length - 1];break;
        case 13:
          if (current.href) window.location = current.href;return;
      }
    } else {
      switch (e.keyCode) {
        case 40:
          next = items[0];break;
        case 38:
          next = items[items.length - 1];break;
      }
    }
